...
<div id="em-booking" class="em-booking <?php if ( get_option( 'dbem_css_rsvp' ) ) {
	echo 'css-booking';
} ?>">
	<?php
	...
	<?php else: ?>
		...
		<?php if ( $tickets_count > 0 ) : ?>
			...
			<form class="em-booking-form" name='booking-form' method='post'
				...
				<?php if ( $can_book ): ?>
					<div class='em-booking-form-details'>
						...
						if ( $show_tickets && $available_tickets_count == 1 && ! get_option( 'dbem_bookings_tickets_single_form' ) ) {
							...
							em_locate_template( 'forms/bookingform/ticket-single.php', true, array(
								'EM_Event'  => $EM_Event,
								'EM_Ticket' => $EM_Ticket,
							) );
							do_action( 'em_booking_form_after_tickets', $EM_Event ); //do not delete
						...
						<div class="em-booking-buttons">
							<?php if ( preg_match( '/https?:\/\//', get_option( 'dbem_bookings_submit_button' ) ) ): //Settings have an image url (we assume). Use it here as the button.?>
								<input type="image" src="<?php echo get_option( 'dbem_bookings_submit_button' ); ?>" class="em-booking-submit"
								       id="em-booking-submit"/>
							<?php else: //Display normal submit button ?>
								<input type="submit" class="em-booking-submit" id="em-booking-submit"
								       value="<?php echo esc_attr( get_option( 'dbem_bookings_submit_button' ) ); ?>"/>
							<?php endif; ?>
						</div>

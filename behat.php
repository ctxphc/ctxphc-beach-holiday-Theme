...
if (version_compare($version, "3.0") >= 0) {
    ...
    final class PhpStormBehatFormatter implements Behat\Testwork\Output\Formatter
    {
	    ...
	    /**
	     * @return \Behat\Behat\Output\Statistics\Statistics|\Behat\Behat\Output\Statistics\TotalStatistics
	     */
        ...
        public function notifyListeners(Symfony\Component\EventDispatcher\Event $event) {
            $eventName = method_exists($event, "getName") ? $event->getName() : "";
            foreach ($this->listeners as $l) {
                $l->listenEvent($this, $event, $eventName);
            }
        }
    ...
    class PhpStormBehatFormatter extends \Behat\Behat\Formatter\PrettyFormatter
    {
        ...
        {
            $this->printStartScenarioEventByName($event->getOutline(), $this->getExampleName($event));
            $this->printProgressType("testStarted");
        }
        ...
        {
            $this->printStartScenarioEventByName($node, $this->getFeatureOrScenarioName($node));
        }

        private function printStartScenarioEventByName(Behat\Gherkin\Node\AbstractScenarioNode $scenario, $name)
        {
